...
**Behavior:** 
- `hashi file.zip --verify` validates embedded CRC32 checksums
- Works with boolean output: `hashi file.zip --verify -b` but still returns warning
...
## Post-Release Security Considerations

### Threat Modeling (FUTURE PRIORITY)
**Rationale:** "A CLI tool that processes hash algorithms and lends itself to automation is the natural prey of bad actors. Should hashi receive any amount of adoption, this danger increases exponentially. We have the responsibility to anticipate threats and proactively block them."

**Post-release security review should include:**
- DoS via resource exhaustion attacks
- Input validation vulnerabilities  
- Integration with planned fuzzing infrastructure
- TOCTOU considerations for file operations
- Recursive reference limits and depth bombing

**Integration:** Coordinate with planned fuzzing machine for comprehensive security testing
## Operational Principles

### Read-Only Stance (ACCEPTED)
**Principle:** Hashi is strictly read-only and will not modify:
- Its own configuration files
- Environment variables  
- Source files it verifies
- System state

**Rationale:** Enhances security, simplifies design, and prevents unintended side effects. All configuration changes must be performed manually by users.

**Implementation:** Ensure no write operations to config files, no environment variable modifications, no file system changes beyond reading.
### JSON Metadata Enhancement (ACCEPTED)
**Processing Timestamp:** Always include processing timestamp in JSON output
- Enables user-side TOCTOU detection
- Creates verifiable audit trail
- Standard practice for CLI tools

**File Metadata:** Include file metadata conditionally
- When filtering by size: Include file sizes in JSON
- When filtering by date: Include modification times in JSON  
- When no filtering active: Keep output clean (no metadata)
- Optional `--include-metadata` flag for metadata without filtering

**Security Philosophy:** "We can't handle all the security on our own. But we should provide an easy environment for users to consider security on their own, when that is reasonable and feasible for the hashi project."
...
### Built-in Pagination (NICE-TO-HAVE)
**Concept:** Human-readable outputs displayed with built-in paging (similar to `| more`)
**Requirements if implemented:**
- Must not threaten automation or scripting compatibility
- Should be controlled via environment variables rather than flags
- Only activate for TTY output of human-readable formats (not JSON/plain)
- Must have clear escape mechanisms for users who prefer standard output
**Rationale:** Could improve usability for large result sets, but automation safety is paramount
### JSON Output Policy (ACCEPTED)
**Core Principle:** "A hashi JSON always acts like a hashi JSON"
- Consistent base structure across all operations
- Predictable parsing for all use cases
- Users learn one JSON schema that works everywhere
- Additive metadata only (never remove fields)

**Possible/Suggested Base JSON Structure:**
```json
{
  "timestamp": "2025-01-17T10:30:45Z",
  "algorithm": "sha256", 
  "operation": "hash|compare|validate",
  "results": [...],
  "metadata": {...}
}
```

**Implementation Priority:** Define JSON schema early in development, after theory/planning phase

### Custom JSON Templates (v1.1 FEATURE)
**Concept:** Environment variables to customize JSON output structure
**Rationale:** Could be super powerful for automation while maintaining consistent default behavior
**Scope:** Post-v1.0 feature for advanced users who need specialized JSON formats