...
### Added
- Implemented Dry Run mode (`--dry-run`) for operation preview and time estimation.
- Implemented Manifest system for incremental hashing (`--manifest`, `--only-changed`, `--output-manifest`).
- Added JSONL output format (`--jsonl`) for streaming and high-performance processing.
- Enhanced `OutputManager` with atomic writes (temp file + rename) for improved safety.
- Added comprehensive unit tests for Manifest and Filtering systems.
- Created detailed user documentation for Filtering, Incremental Hashing, and Dry Run modes.
